    ...
    @if (cuenta()) {
      <div class="max-w-4xl">
        ...
        <div class="mb-6 flex justify-between items-start">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Información detallada de la cuenta</p>
          </div>
          ...
          <div class="px-6 py-4">
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <dd class="mt-1 text-sm text-gray-900 font-mono">{{ cuenta()!.numeroCuenta }}</dd>
              </div>
              <div>
                <dt class="text-sm font-medium text-gray-500">Tipo de Cuenta</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ getTipoCuentaLabel(cuenta()!.tipoCuenta) }}</dd>
              </div>
              <div>
              ...
              <div>
                ...
                <dd class="mt-1 text-sm text-gray-900 font-mono">{{ cuenta()!.cuentaId }}</dd>
              </div>
            </dl>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
          <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Titular de la Cuenta</h2>
          </div>
          <div class="px-6 py-4">
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <dt class="text-sm font-medium text-gray-500">Nombre Completo</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ cuenta()!.cliente.nombre }}</dd>
              </div>
              <div>
                <dt class="text-sm font-medium text-gray-500">Identificación</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ cuenta()!.cliente.identificacion }}</dd>
              </div>
              <div>
                <dt class="text-sm font-medium text-gray-500">Teléfono</dt>
                <dd class="mt-1 text-sm text-gray-900">
                  <a [href]="'tel:' + cuenta()!.cliente.telefono" class="text-blue-600 hover:text-blue-800">
              ...
              <div class="md:col-span-2">
                ...
                <dd class="mt-1 text-sm text-gray-900">{{ cuenta()!.cliente.direccion }}</dd>
              </div>
            </dl>
          </div>
        </div>
      ...
      <div class="text-center py-12">
        @if (cuentasService.loading()) {
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900">Cuenta no encontrada</h3>
          <p class="mt-1 text-sm text-gray-500">La cuenta que buscas no existe o fue eliminada.</p>
          <div class="mt-6">
            <app-button
              [variant]="'primary'"
              (clicked)="goBack()"
...
export class CuentaDetailComponent implements OnInit {
  ...
  editCuenta(): void {
    if (this.cuenta()) {
      this.router.navigate(['/cuentas', this.cuenta()!.cuentaId, 'editar']);
    }
  }
